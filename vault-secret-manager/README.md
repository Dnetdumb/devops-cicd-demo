...
```bash
	Postgres (CNPG)
	  |	Dynamic users
	  ▼
        ESO 	Send request to Vault)
	  |
	  ▼
	Vault
	  |	Create user with dynamic secret (secret have interval)
	  ▼
	ESO (External Secret Operator on K8s)
	  |	Sync to K8s secret
	  ▼
	Pod (load secret or restart with reloader:true )
```
